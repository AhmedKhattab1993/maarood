...
export function SaveButton({
  ...
  useEffect(() => {
    ...
    void listSaved()
      .then((items) => {
        setSaved(isProductSaved(productId, { savedIds: savedProductIds(items) }));
      })
    ...
    return (
      <span className="inline-flex flex-col gap-1">
        <button
          ...
          className={`min-h-12 rounded-default border px-4 py-2 text-sm font-medium transition-colors disabled:opacity-50 ${labelClass}`}
        >
          {visual === 'pending' ? t('savePending') : saved ? t('saved') : t('save')}
        </button>
        {visual === 'failed' && (
