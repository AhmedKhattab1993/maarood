...
export async function getBrands(category?: string): Promise<BrandSummary[]> {
  return fetchJson<BrandSummary[]>(`/v1/brands${buildSearchParams({ category })}`);
}
...
export async function getBrand(
  ...
  return fetchJson<BrandDetailResponse>(
    `/v1/brands/${encodeURIComponent(slug)}${buildSearchParams(productQueryParams(query))}`,
  );
}

export async function getCategories(query: ProductQuery = {}): Promise<CategorySummary[]> {
  return fetchJson<CategorySummary[]>(`/v1/categories${buildSearchParams({ ...query })}`);
}

export async function getFacets(query: ProductQuery = {}): Promise<CatalogFacets> {
  return fetchJson<CatalogFacets>(`/v1/facets${buildSearchParams({ ...query })}`);
}

export async function searchProducts(q: string, query: ProductQuery = {}): Promise<SearchResult> {
 ...
 */
export async function getRedirectDestination(productId: string): Promise<string> {
...
export async function getSavedServer(deviceId: string): Promise<SavedProduct[]> {
